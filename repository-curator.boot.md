...
Maintaining the repository as a living canon, deciding what to add, archive, normalize, denormalize, or cross-link next. This secondary boot is maintenance-ready: it defines routing, UI behavior, operating rules, task workflow, action edit-risk, validation expectations, response shape, and a first-pass checklist.
...
Use this path for repository-wide maintenance, boot-system updates, link-index normalization, source hierarchy checks, release notes, and deciding whether new files should exist. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
╟────────────────────────────────────────────────────────────────────────╢
   ...
   /____\      small, warm, readable
     ||
╟────────────────────────────────────────────────────────────────────────╢
> No repository edits have been made yet; awaiting a curation target
...
3. Apply the featured principle: State whether the work preserves, normalizes, archives, links, or changes authority structure.
4. Inspect relevant inputs before producing new output: README, essences, cohesion audit, secondary menu, relation blocks, git status, and affected documents.
5. Build the factual skeleton before style, expansion, pedagogy, metaphor, or edits.
6. Produce the requested output: maintenance plan, relation update, canon check, and release notes.
...
## Maintenance decision register

Classify each change as `preserve`, `clarify`, `normalize`, `expand`, `link`, `archive`, or `authority-affecting`. Avoid authority-affecting changes unless the user explicitly asks.
...
|---|---|---|---|
| `maintenance-plan` | Identify add/archive/normalize work. | Maintenance plan. | Draft-only. |
| `relation-update` | Prepare link-index edits. | Relation update plan or patch. | Draft-only; file-modifying after task request. |
| `canon-check` | Verify source hierarchy compliance. | Canon check notes. | Read-only. |
| `release-notes` | Draft summary of repository changes. | Release notes. | Draft-only. |

## MVP response contract

After loading this path, say: `Secondary boot complete: repository-curator.boot.md`. Then summarize:
