...
Working with extended tables, first-100-step ratios, canonical-vs-extended ranges, and workbook-derived calculations. This secondary boot is maintenance-ready: it defines routing, UI behavior, operating rules, task workflow, action edit-risk, validation expectations, response shape, and a first-pass checklist.
...
Use this path for workbook inspection, recomputation, CSV/table export, column maps, extended ratios, canonical-vs-extended range declarations, and spreadsheet-derived assertions. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
╟────────────────────────────────────────────────────────────────────────╢
   ...
   /____\      small, warm, readable
     ||
╟────────────────────────────────────────────────────────────────────────╢
> No data table or export has been created yet; awaiting a workbook or
> range task.
...
4. Inspect relevant inputs before producing new output: workbook sheets, formulas, column meanings, range declarations, glossary terms, and canonical destination table.
...
Before trusting workbook-derived values, identify sheet names, columns, formula lineage, range endpoints, and whether labels are repository Fv labels or converted conventional indices.
...
|---|---|---|---|
| `range-declare` | State canonical vs extended range. | Range note. | Read-only. |
| `column-map` | Map spreadsheet columns to repository terms. | Column map. | Read-only. |
| `recompute-check` | Plan or run formula validation. | Recompute checklist/results. | Read-only unless exporting files. |
| `export-plan` | Define CSV/table outputs. | Export plan. | Draft-only; file-modifying only after request. |

## MVP response contract

After loading this path, say: `Secondary boot complete: spreadsheet-data.boot.md`. Then summarize:
