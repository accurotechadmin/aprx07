...
Working with extended tables, first-100-step ratios, canonical-vs-extended ranges, and workbook-derived calculations. This first-draft secondary boot is intentionally MVP-level: complete enough to enter the mode safely, compact enough to refine later.

## When to use

Use this path when the user's task primarily needs: working with extended tables, first-100-step ratios, canonical-vs-extended ranges, and workbook-derived calculations. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.

## Required reading order

1. [boot.md](boot.md) for primary boot state.
2. [the_essences.md](the_essences.md) for source hierarchy and canon.
3. [fibonacci_first_100_with_ratios.xlsx](fibonacci_first_100_with_ratios.xlsx)
...
|---|---|---|---|
| `range-declare` | State canonical vs extended range. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `column-map` | Map spreadsheet columns to terms. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `recompute-check` | Plan formula validation. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `export-plan` | Define CSV/table outputs. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |

## MVP response contract

After loading this path, say: `Secondary boot complete: spreadsheet-data.boot.md`. Then summarize the active sources, selected actions, convention/range assumptions, and the next concrete step.
