# orange-pressing-expander.boot.md
...
Expanding small insights into families of documents, graphs, data manifests, workbooks, lessons, and future creative branches. This secondary boot is maintenance-ready: it defines routing, UI behavior, operating rules, task workflow, action edit-risk, validation expectations, response shape, and a first-pass checklist.

## When to use

Use this path when the user wants many derived ideas, artifact families, unusual extensions, or creative branches that must return to canonical exactness. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
3. Apply the featured principle: Map every creative branch back to a canonical source, caveat, or exact structure.
4. Inspect relevant inputs before producing new output: orange recommendations, generation lattice, graph suite, drawing guide, and any exact seed insight being expanded.
5. Build the factual skeleton before style, expansion, pedagogy, metaphor, or edits.
6. Produce the requested output: branch map, artifact list, priority sort, and return-to-core map.
...
Sort ideas into lanes: exact math, visual proof, lesson, dataset, metaphor, empirical caution, repository maintenance, and expressive art. Every lane needs a caveat strategy.
...
|---|---|---|---|
| `branch-map` | Generate expansion branches. | Branch map. | Draft-only. |
| `artifact-list` | Propose documents, graphs, datasets, and lessons. | Artifact list. | Draft-only. |
| `priority-sort` | Rank expansions by value and readiness. | Priority table. | Read-only or draft-only. |
| `return-to-core` | Map creative ideas back to exactness. | Core-return table. | Read-only. |

## MVP response contract

After loading this path, say: `Secondary boot complete: orange-pressing-expander.boot.md`. Then summarize:
