...
Exact Fibonacci values, recurrence, ratios, limits, proofs, convention conversions, and finite/exact distinction. This first-draft secondary boot is intentionally MVP-level: complete enough to enter the mode safely, compact enough to refine later.

## When to use

Use this path when the user's task primarily needs: exact fibonacci values, recurrence, ratios, limits, proofs, convention conversions, and finite/exact distinction. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
5. [Fibonacci_Glossary.md](Fibonacci_Glossary.md)
6. [Fibonacci_Cohesion_Audit.md](Fibonacci_Cohesion_Audit.md)
...
3. Apply the featured principle: Make exact identities, finite values, and limits visibly distinct.
4. Inspect relevant inputs before producing new output: truth inventory, avoids inventory, glossary terms, and cohesion notes.
5. Build the factual skeleton before style, expansion, pedagogy, metaphor, or edits.
6. Produce the requested output: formula sheet, conversion notes, proof frame, and edge-case cautions.
...
|---|---|---|---|
| `formula-sheet` | Summarize active formulas before answering. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `index-convert` | Prepare Fv/conventional conversion notes. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `proof-mode` | Use proof-first response framing. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `edge-cases` | List exactness and finite-ratio cautions. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |

## MVP response contract

After loading this path, say: `Secondary boot complete: truth-inventory-mathematician.boot.md`. Then summarize the active sources, selected actions, convention/range assumptions, and the next concrete step.
