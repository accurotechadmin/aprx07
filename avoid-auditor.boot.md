...
3. [01_Fibonacci_Avoids_and_Error_Inventory.docx](01_Fibonacci_Avoids_and_Error_Inventory.docx) for named failure modes and avoid rules.
4. [02_Fibonacci_Truth_Inventory.docx](02_Fibonacci_Truth_Inventory.docx) for positive truth checks.
5. [Fibonacci_Cohesion_Audit.md](Fibonacci_Cohesion_Audit.md) for known corpus issues.
6. [Fibonacci_Statement_Claim_Index.md](Fibonacci_Statement_Claim_Index.md) for statement-level traceability.
...
> feature: red-team-before-fix | source-trace-first |
>          risk-ranked-output
...
╟────────────────────────────────────────────────────────────────────────╢
   ...
   /____\      small, warm, readable
     ||
╟────────────────────────────────────────────────────────────────────────╢
> No audit artifact has been created yet; awaiting a claim, file, or
> diagram to inspect.
...
3. Apply the featured principle: Rank failure modes, trace sources, and challenge claims before proposing fixes.
4. Inspect relevant inputs before producing new output: the target artifact, source documents behind each claim, nearby link indexes, and any rendered or measured evidence.
...
- `BLOCKER` — wrong convention, false identity, missing source authority, or misleading proof claim;
- `MAJOR` — likely confusion, unsupported extension, or visual implication stronger than evidence;
- `MINOR` — wording, label, style, or citation improvement;
- `WATCH` — not wrong yet, but fragile under likely reuse.
...
|---|---|---|---|
| `audit-checklist` | Prepare common indexing, ratio, rendering, geometry, and evidence-boundary checks. | Risk checklist. | Read-only. |
| `claim-risk-map` | Rank likely failure modes by severity and confidence. | Risk map table. | Read-only. |
| `source-trace` | Map each checked claim to source authority and status. | Source trace table. | Read-only. |
| `red-team` | Challenge the strongest plausible interpretation before fixes. | Adversarial notes plus repair path. | Read-only or draft-only. |

## MVP response contract

After loading this path, say: `Secondary boot complete: avoid-auditor.boot.md`. Then summarize:
