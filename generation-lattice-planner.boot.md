# generation-lattice-planner.boot.md
...
Planning next documents and figures by horizontal topic family and vertical development level. This secondary boot is maintenance-ready: it defines routing, UI behavior, operating rules, task workflow, action edit-risk, validation expectations, response shape, and a first-pass checklist.

## When to use

Use this path for placing work in the generation lattice, planning prerequisites, recommending next artifacts, or preparing Markdown/JSON manifest updates. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
╟────────────────────────────────────────────────────────────────────────╢
   ...
   /____\      small, warm, readable
     ||
╟────────────────────────────────────────────────────────────────────────╢
> No lattice plan has been created yet; awaiting a planning target or
> artifact family.
...
3. Apply the featured principle: Locate work in the lattice before adding files, so every artifact has a reason to exist.
4. Inspect relevant inputs before producing new output: generation lattice Markdown/JSON, curated graph list, orange recommendations, and existing files that may satisfy a proposed slot.
5. Build the factual skeleton before style, expansion, pedagogy, metaphor, or edits.
6. Produce the requested output: lattice position, next-three artifacts, dependency map, and manifest draft.
...
## Lattice planning schema

For each candidate, record: `id`, `path`, `family`, `level`, `artifact type`, `prerequisites`, `source lineage`, `why now`, and `done check`.
...
|---|---|---|---|
| `lattice-position` | Place work on horizontal/vertical lattice. | Lattice coordinate note. | Read-only. |
| `next-three` | Propose next three artifacts. | Ranked next-three list. | Draft-only. |
| `dependency-map` | List prerequisites. | Dependency map. | Read-only. |
| `manifest-draft` | Draft JSON/Markdown updates. | Manifest patch plan. | Draft-only; file-modifying only after request. |

## MVP response contract

After loading this path, say: `Secondary boot complete: generation-lattice-planner.boot.md`. Then summarize:
