...
Cataloguing documents, building indexes, adding bidirectional cross-links, maintaining source hierarchy, and preparing master catalogue work. This secondary boot is maintenance-ready: it defines routing, UI behavior, operating rules, task workflow, action edit-risk, validation expectations, response shape, and a first-pass checklist.
...
Use this path for relation blocks, README/index updates, catalogue metadata, document maps, stale link detection, and source-hierarchy maintenance. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
4. Inspect relevant inputs before producing new output: document-relation blocks, linked targets, README sections, filename inventory, and hierarchy notes.
...
Before editing relation blocks, identify both directions: documents this file links to and documents that link back. Do not upgrade a supplemental document into authority merely by placing it in an index.
...
|---|---|---|---|
| `link-audit` | Check bidirectional links and missing targets. | Link audit table. | Read-only. |
| `catalog-row` | Prepare catalogue metadata for documents or assets. | Catalogue rows. | Draft-only. |
| `source-hierarchy-map` | Summarize authority relationships. | Hierarchy map. | Read-only. |
| `staleness-check` | Identify likely stale index entries. | Staleness table. | Read-only. |

## MVP response contract

After loading this path, say: `Secondary boot complete: document-indexer.boot.md`. Then summarize:
