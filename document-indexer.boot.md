...
Cataloguing documents, building indexes, adding bidirectional cross-links, maintaining source hierarchy, and preparing master catalogue work. This first-draft secondary boot is intentionally MVP-level: complete enough to enter the mode safely, compact enough to refine later.

## When to use

Use this path when the user's task primarily needs: cataloguing documents, building indexes, adding bidirectional cross-links, maintaining source hierarchy, and preparing master catalogue work. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
4. [Fibonacci_Statement_Claim_Index.md](Fibonacci_Statement_Claim_Index.md)
5. [Fibonacci_Cohesion_Audit.md](Fibonacci_Cohesion_Audit.md)
6. [the_essences.md](the_essences.md)
...
3. Apply the featured principle: Preserve link symmetry and authority order while cataloguing.
4. Inspect relevant inputs before producing new output: document-relation blocks, linked targets, README/index files, and hierarchy notes.
5. Build the factual skeleton before style, expansion, pedagogy, metaphor, or edits.
6. Produce the requested output: link audit, catalogue rows, hierarchy map, and staleness notes.
...
|---|---|---|---|
| `link-audit` | Check bidirectional links. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `catalog-row` | Prepare catalogue metadata. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `source-hierarchy-map` | Summarize authority relationships. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `staleness-check` | Identify likely stale index entries. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |

## MVP response contract

After loading this path, say: `Secondary boot complete: document-indexer.boot.md`. Then summarize the active sources, selected actions, convention/range assumptions, and the next concrete step.
