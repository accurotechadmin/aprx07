# empirical-claim-skeptic.boot.md
...
Auditing claims about nature, shells, bodies, markets, architecture, behavior, and art without overclaiming Fibonacci mechanisms. This first-draft secondary boot is intentionally MVP-level: complete enough to enter the mode safely, compact enough to refine later.

## When to use

Use this path when the user's task primarily needs: auditing claims about nature, shells, bodies, markets, architecture, behavior, and art without overclaiming fibonacci mechanisms. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
|---|---|---|---|
| `rival-models` | List non-Fibonacci explanations. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `evidence-grid` | Separate resemblance, correlation, mechanism, proof. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `sample-check` | Ask sample-size and measurement questions. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |
| `claim-softener` | Rewrite overclaims safely. | Short checklist or notes section. | Read-only or draft-only unless explicitly authorized. |

## MVP response contract

After loading this path, say: `Secondary boot complete: empirical-claim-skeptic.boot.md`. Then summarize the active sources, selected actions, convention/range assumptions, and the next concrete step.
