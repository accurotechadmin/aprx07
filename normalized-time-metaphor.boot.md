...
Use this path for phi-scaled time framing, planning horizons, reciprocal-time mirrors, salience diagrams, memory/anticipation metaphors, and philosophical time visuals. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
> feature: metaphor-badge | normalized-quantity-named |
>          asymmetry-visible
...
╟────────────────────────────────────────────────────────────────────────╢
   ...
   /____\      small, warm, readable
     ||
╟────────────────────────────────────────────────────────────────────────╢
> No metaphor card or graphic has been created yet; awaiting a
> normalized quantity or time task.
...
3. Apply the featured principle: Name the normalized quantity and metaphor status before interpreting phi positions.
4. Inspect relevant inputs before producing new output: the essences, graph suite, generation lattice time rows, orange-pressing ideas, and the user’s intended normalized quantity.
...
## Time-metaphor contract

Default positions are Yesterday = `1/phi`, Today = `1`, Tomorrow = `phi`, but this is an interpretive metaphor. The intervals are asymmetric; do not imply equal spacing around today.
...
|---|---|---|---|
| `metaphor-badge` | State metaphor status and normalized quantity. | Badge and framing note. | Read-only. |
| `asymmetry-note` | Include interval asymmetry around today. | Asymmetry calculation note. | Read-only. |
| `concept-map` | Draft planning/salience/memory axes. | Concept map. | Draft-only. |
| `caveat-card` | List what the metaphor does not prove. | Caveat card. | Read-only. |

## MVP response contract

After loading this path, say: `Secondary boot complete: normalized-time-metaphor.boot.md`. Then summarize:
