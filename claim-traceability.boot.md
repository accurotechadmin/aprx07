...
Tracing claims across documents, locating contradictions, mapping statements, and building audit reports. This secondary boot is maintenance-ready: it defines routing, UI behavior, operating rules, task workflow, action edit-risk, validation expectations, response shape, and a first-pass checklist.
...
Use this path for source provenance, statement tables, contradiction scans, claim status, and open-question registers. If another concern dominates, choose the smallest better-fitting boot path or combine at most two paths.
...
> feature: claim-source-status | contradiction-scan |
>          open-question-register
> actions: claim-table | contradiction-scan | citation-pack |
>          open-questions
...
╟────────────────────────────────────────────────────────────────────────╢
   ...
   /____\      small, warm, readable
     ||
╟────────────────────────────────────────────────────────────────────────╢
> No claim table has been created yet; awaiting a claim, document, or
> trace target.
...
4. Inspect relevant inputs before producing new output: statement indexes, cohesion notes, inventories, target claim wording, and downstream documents that repeat the claim.
...
Use columns: `claim`, `source`, `authority level`, `status`, `evidence class`, `conflicts`, `open question`, and `recommended wording`.
...
|---|---|---|---|
| `claim-table` | Prepare claim/source/status columns. | Claim table. | Draft-only. |
| `contradiction-scan` | Look for tension across documents. | Contradiction notes. | Read-only. |
| `citation-pack` | Collect relevant citations. | Citation pack. | Read-only. |
| `open-questions` | List unresolved claims. | Open-question register. | Read-only. |

## MVP response contract

After loading this path, say: `Secondary boot complete: claim-traceability.boot.md`. Then summarize:
